...
(function installBrasfelsDashboard() {
  ...
  function createView() {
    if (document.querySelector('#view-dashboard')) return;
    ...
    section.innerHTML = `
      <div class="dashboard-hero">
        ...
        <div class="dashboard-hero-side">
          <div class="dashboard-source-card"><span>Fonte operacional</span><strong id="dashboardSourceName">Joint Traceability + Spool Map</strong><small id="dashboardSourceTime">Aguardando carregamento...</small></div>
          <div class="dashboard-actions"><button class="button secondary" id="dashboardAuditSource">Abrir base</button><button class="button secondary" id="dashboardRefresh">Atualizar</button></div>
        </div>
      </div>
      <div class="dashboard-filterbar">
        <label class="dashboard-filter"><span>Módulo</span><select id="dashboardModule"><option value="">Todos os módulos</option></select></label>
        <label class="dashboard-filter"><span>Local de execução</span><select id="dashboardPlacement"><option value="">PIPE + CAMPO</option><option value="PIPE">PIPE</option><option value="CAMPO">CAMPO</option></select></label>
        <label class="dashboard-filter"><span>Semana de análise</span><select id="dashboardWeek"><option value="">Última semana</option></select></label>
        <label class="dashboard-filter"><span>Janela dos gráficos</span><select id="dashboardPeriod"><option value="12">12 semanas</option><option value="16" selected>16 semanas</option><option value="24">24 semanas</option><option value="all">Todo histórico</option></select></label>
      </div>
      <div id="dashboardContent"><div class="db-empty"><div><strong>Carregando Dashboard...</strong><span>Conferindo Joint Traceability, Spool Map e escopo.</span></div></div></div>`;
  ...
  function filteredRows() {
    const moduleRows = joints.filter(row => !filters.module || upper(row.module) === filters.module);
  ...
  function compute() {
    ...
    for (const row of pipeRows) {
      ...
      if (spool && totalPipe) weldedWeightByWeek.set(wk, (weldedWeightByWeek.get(wk) || 0) + number(spool.weight_kg) / totalPipe);
    }
    ...
    for (const [key, spoolRows] of rowsBySpool.entries()) {
      const pipe = spoolRows.filter(row => upper(row.placement) === 'PIPE');
      ...
      const pending = pipe.length - approved.length;
      pendingDimensionalJoints += pending;
      const pendingWeight = pipe.length ? weight * pending / pipe.length : 0;
      if (spool?.on_hold || /HOLD/.test(upper(spool?.manufacture_status))) holdPendingWeight += pendingWeight;
      else pendingFabricationWeight += pendingWeight;
      if (pending === 0) {
        const dates = pipe.map(row => parseDate(row.dimensional_date)).filter(Boolean);
        const completedAt = dates.length ? new Date(Math.max(...dates.map(d => d.getTime()))) : null;
        fabricated.push({ key, completedAt, weight, joints: pipe.length });
    ...
    let actualUsed = 0; let plannedUsed = 0;
    const rundown = weightWeekly.map(row => {
      actualUsed += row.actual; plannedUsed += row.planned;
      return { ...row, actualRemaining: Math.max(0, totalWeight - actualUsed), plannedRemaining: Math.max(0, totalWeight - plannedUsed) };
    });
  ...
  function lineChart(target, rows, series, formatter = value => fmt(value)) {
    ...
    if (!el) return;
    if (!rows.length) { el.innerHTML = '<div class="db-empty"><div><strong>Sem dados para este filtro.</strong></div></div>'; return; }
    ...
    let html = `<div class="db-legend">${series.map(s => `<span><i></i>${escape(s.label)}</span>`).join('')}</div><svg viewBox="0 0 ${width} ${height}" role="img">`;
    for (let t = 0; t <= 4; t += 1) {
      const val = max * t / 4, yy = top + ph - t * ph / 4;
      html += `<line class="db-gridline" x1="${left}" y1="${yy}" x2="${width-right}" y2="${yy}"></line><text class="db-axis" x="${left-8}" y="${yy+4}" text-anchor="end">${escape(formatter(val))}</text>`;
    }
    data.forEach((row, i) => {
      if (data.length <= 18 || i % 2 === 0 || i === data.length - 1) html += `<text class="db-axis" x="${x(i)}" y="${height-12}" text-anchor="middle">${escape(row.label)}</text>`;
    });
    series.forEach((s, si) => {
      const pts = data.map((row, i) => `${x(i)},${y(row[s.key])}`).join(' ');
      html += `<polyline class="db-line-${si}" points="${pts}"></polyline>`;
      data.forEach((row, i) => html += `<circle class="db-dot-${si}" cx="${x(i)}" cy="${y(row[s.key])}" r="3.5"><title>${escape(`${s.label} · ${row.label}: ${formatter(row[s.key])}`)}</title></circle>`);
    });
    html += '</svg>';
    el.innerHTML = html;
  }
  ...
  function barChart(target, rows, formatter = value => fmt(value)) {
    const el = document.querySelector(target);
    if (!el) return;
    if (!rows.length) { el.innerHTML = '<div class="db-empty"><strong>Sem dados.</strong></div>'; return; }
    ...
    for (let t=0;t<=4;t+=1){const val=max*t/4,yy=top+ph-t*ph/4;html+=`<line class="db-gridline" x1="${left}" y1="${yy}" x2="${width-right}" y2="${yy}"></line><text class="db-axis" x="${left-7}" y="${yy+4}" text-anchor="end">${escape(formatter(val))}</text>`;}
    rows.forEach((row,i)=>{const v=number(row.value),h=v/max*ph,x=left+slot*i+(slot-bw)/2,y=top+ph-h;html+=`<rect class="db-bar" x="${x}" y="${y}" width="${bw}" height="${h}" rx="5"><title>${escape(`${row.label}: ${formatter(v)}`)}</title></rect><text class="db-axis" x="${x+bw/2}" y="${height-11}" text-anchor="middle">${escape(row.label)}</text>`;});
  ...
  function listBars(target, rows) {
    ...
    el.innerHTML=`<div class="db-stage-list">${rows.map(r=>`<div class="db-stage-row"><span title="${escape(r.label)}">${escape(r.label)}</span><div class="db-track"><i style="width:${Math.min(100,r.value/max*100)}%"></i></div><strong>${fmt(r.value)}</strong></div>`).join('')}</div>`;
  }

  function statusBars(target, rows) {
    const el=document.querySelector(target); if(!el)return;
    const shown=rows.slice(0,12),max=Math.max(1,...shown.map(r=>r.value));
    el.innerHTML=`<div class="db-status-list">${shown.map(r=>`<div class="db-status-row"><span title="${escape(r.label)}">${escape(r.label)}</span><div class="db-track"><i style="width:${Math.min(100,r.value/max*100)}%"></i></div><strong>${fmt(r.value)}</strong></div>`).join('')}</div>`;
  }

  function render() {
    const content=document.querySelector('#dashboardContent'); if(!content)return;
    if(!joints.length){content.innerHTML='<div class="db-empty"><div><strong>Joint Traceability P85 ainda não está disponível.</strong><span>O Dashboard não utiliza os números da planilha de gráficos; ele precisa da base operacional atual.</span></div></div>';return;}
    const d=compute();
    ...
    content.innerHTML=`
      <div class="dashboard-kpis">
        <article class="dashboard-kpi success"><span>Juntas liberadas</span><strong>${fmt(d.releasedJoints)}</strong><small>Data de liberação / situação atual</small></article>
        <article class="dashboard-kpi success"><span>Spools 100% dimensional</span><strong>${fmt(d.fabricated.length)}</strong><small>${fmt(fabricationPct,1)}% dos spools PIPE elegíveis</small></article>
        <article class="dashboard-kpi"><span>Peso fabricado</span><strong>${fmt(d.fabricatedWeight,2)} t</strong><small>Soma do peso dos spools concluídos</small></article>
        <article class="dashboard-kpi"><span>Juntas soldadas</span><strong>${fmt(d.weldedJointsTotal)}</strong><small>Acumulado de juntas PIPE</small></article>
        <article class="dashboard-kpi warn"><span>Backlog fabricação</span><strong>${fmt(d.pendingFabricationWeight,2)} t</strong><small>${fmt(d.pendingDimensionalJoints)} juntas PIPE pendentes</small></article>
        <article class="dashboard-kpi warn"><span>Peso em hold</span><strong>${fmt(d.holdPendingWeight,2)} t</strong><small>Peso pendente proporcional em spools hold</small></article>
        <article class="dashboard-kpi"><span>Aptos para pintura</span><strong>${fmt(d.aptPainting)}</strong><small>Status: Aguardando Pintura de Fundo</small></article>
        <article class="dashboard-kpi ${d.pendingTraceability?'danger':'success'}"><span>Rastreabilidade pendente</span><strong>${fmt(d.pendingTraceability)}</strong><small>Dimensional aprovado com pendência de material</small></article>
      </div>

      <div class="dashboard-section"><div class="dashboard-section-head"><div><p class="eyebrow">PRODUÇÃO</p><h3>Evolução e desempenho de fábrica</h3></div><p>${escape(weekLabel(filters.week))}</p></div>
        <div class="dashboard-grid">
          <article class="dashboard-card span-12"><div class="dashboard-card-head"><div><h4>Evolução semanal · quantidade</h4><p>Reprodução do gráfico “Evolução Semanal (QTD)”: Corte, Montagem e Soldagem.</p></div><span class="dashboard-chip">Relatório 1/10</span></div><div id="dbStageWeekly" class="dashboard-chart"></div></article>
          <article class="dashboard-card"><div class="dashboard-card-head"><div><h4>Quantidade acumulada por etapa</h4><p>Oito etapas do fluxo de fabricação.</p></div><span class="dashboard-chip">Relatório 3/10</span></div><div id="dbCumulative" class="dashboard-chart compact"></div></article>
          <article class="dashboard-card"><div class="dashboard-card-head"><div><h4>Quantidade da semana selecionada</h4><p>Mesma leitura do gráfico semanal da planilha, sem valores congelados.</p></div><span class="dashboard-chip">Relatório 5/10</span></div><div id="dbWeekStages" class="dashboard-chart compact"></div></article>
          <article class="dashboard-card"><div class="dashboard-card-head"><div><h4>Polegadas soldadas por dia</h4><p>Soma do diâmetro das juntas PIPE pela data de soldagem.</p></div><span class="dashboard-chip">Relatório 2/10</span></div><div id="dbDailyInches" class="dashboard-chart compact"></div></article>
          <article class="dashboard-card"><div class="dashboard-card-head"><div><h4>Comparativo de desempenho da fábrica</h4><p>O gráfico original mistura três escalas. Aqui elas ficam sincronizadas e separadas para leitura correta.</p></div><span class="dashboard-chip">Relatórios 7–8/10</span></div><div class="db-mini-grid"><div class="db-mini"><h5>Spools fabricados</h5><strong>${fmt(d.selectedProduction.fabricated)}</strong><small>${escape(weekLabel(filters.week,true))}</small><div id="dbMiniSpools"></div></div><div class="db-mini"><h5>Juntas soldadas</h5><strong>${fmt(d.selectedProduction.weldedJoints)}</strong><small>${escape(weekLabel(filters.week,true))}</small><div id="dbMiniJoints"></div></div><div class="db-mini"><h5>Peso soldado</h5><strong>${fmt(d.selectedProduction.weldedWeightT,2)} t</strong><small>${escape(weekLabel(filters.week,true))}</small><div id="dbMiniWeight"></div></div></div></article>
        </div>
      </div>

      <div class="dashboard-section"><div class="dashboard-section-head"><div><p class="eyebrow">PESO & PROGRAMAÇÃO</p><h3>Real, planejado e rundown</h3></div><p>Spool Map + Joint Traceability</p></div>
        <div class="dashboard-grid">
          <article class="dashboard-card span-8"><div class="dashboard-card-head"><div><h4>Peso semanal de juntas soldadas</h4><p>Peso do spool rateado pelas juntas PIPE, agrupado pela semana da soldagem.</p></div><span class="dashboard-chip">Relatório 4/10</span></div><div id="dbWeightWeekly" class="dashboard-chart"></div></article>
          <article class="dashboard-card span-4"><div class="dashboard-card-head"><div><h4>Fabricado x backlog</h4><p>Base: spools com juntas PIPE.</p></div></div><div class="db-ring-wrap"><div class="db-ring" style="--p:${Math.min(100,fabricationPct)}"><div><strong>${fmt(fabricationPct,1)}%</strong><small>fabricado</small></div></div><div class="db-split"><div><span>Fabricados</span><strong>${fmt(d.fabricated.length)}</strong></div><div><span>Backlog</span><strong>${fmt(Math.max(0,d.eligibleSpools-d.fabricated.length))}</strong></div><div><span>Elegíveis</span><strong>${fmt(d.eligibleSpools)}</strong></div></div></div></article>
          <article class="dashboard-card span-8"><div class="dashboard-card-head"><div><h4>Fabrication rundown · peso remanescente</h4><p>Atualização do rundown legado: saldo real comparado ao peso programado no Spool Map, sem usar o planejamento Brasfels obsoleto.</p></div><span class="dashboard-chip">Relatório 9/10</span></div><div id="dbRundown" class="dashboard-chart"></div></article>
          <article class="dashboard-card span-4"><div class="dashboard-card-head"><div><h4>Análise de escopo · status</h4><p>Distribuição atual do Manufacture Status.</p></div><span class="dashboard-chip">Relatório 10/10</span></div><div id="dbStatuses" class="dashboard-chart compact"></div></article>
        </div>
      </div>

      <div class="dashboard-section"><div class="dashboard-section-head"><div><p class="eyebrow">VALOR AGREGADO</p><h3>Liberação técnica valorizada</h3></div><p>Somente quando há vínculo verificável com a tabela de escopo</p></div>
        <div class="dashboard-grid">
          <article class="dashboard-card span-12"><div class="dashboard-card-head"><div><h4>Valor agregado por juntas liberadas</h4><p>O legado valorizava cada junta pela faixa diâmetro/espessura. Aqui a junta atual é vinculada ao escopo e à classe de inspeção antes de receber valor.</p></div><span class="dashboard-chip">Relatório 6/10</span></div><div id="dbValueSeries" class="dashboard-chart"></div>${d.pricedReleased?`<div class="dashboard-note">Cobertura de valorização: <strong>${fmt(d.pricedReleased)}</strong> de <strong>${fmt(d.releasedJoints)}</strong> juntas liberadas (${fmt(priceCoverage,1)}%). Valor acumulado verificável: <strong>${escape(money(d.releasedValue))}</strong>.</div>`:`<div class="dashboard-note warn"><strong>Valor não exibido.</strong> O Dashboard não vai estimar preço médio. É necessário existir vínculo entre Joint Traceability, Base CS e tabela de valores para reproduzir este relatório sem inventar dados.</div>`}</article>
        </div>
      </div>

      <details class="dashboard-audit"><summary>Rastreabilidade dos 10 relatórios analisados</summary><div class="dashboard-audit-grid">
        <div class="dashboard-audit-item"><strong>1 · Evolução semanal</strong><span>Excel: BASE DE DADOS K41/K42/K44 × semanas AI:BC.</span><code>DB: cutting_date + coupling_date + welding_date</code></div>
        <div class="dashboard-audit-item"><strong>2 · Polegadas soldadas / dia</strong><span>Excel: POL SOLDADAS, soma de diâmetro para PIPE pela data de soldagem.</span><code>DB: diameter_inch + welding_date + placement=PIPE</code></div>
        <div class="dashboard-audit-item"><strong>3 · Quantidade acumulada</strong><span>Excel: 8 etapas, BASE DE DADOS K52:L59.</span><code>DB: datas preenchidas das 8 etapas do Joint Traceability</code></div>
        <div class="dashboard-audit-item"><strong>4 · Peso semanal soldado</strong><span>Excel: PESO - JUNTAS SOLDADAS → BASE DE DADOS linha 81.</span><code>DB: weight_kg do spool ÷ juntas PIPE × juntas soldadas</code></div>
        <div class="dashboard-audit-item"><strong>5 · Quantidade da semana</strong><span>Excel: INDEX da matriz semanal K41:BC48.</span><code>DB: mesma contagem das etapas filtrada por WEEKNUM(data,14)-1</code></div>
        <div class="dashboard-audit-item"><strong>6 · Valor agregado</strong><span>Excel: quantidade liberada × valor unitário por diâmetro/espessura.</span><code>DB: release_date + inspection_class + p85_scope_joints + p85_scope_inspection_rates</code></div>
        <div class="dashboard-audit-item"><strong>7–8 · Desempenho de fábrica</strong><span>Excel: gráfico composto com peso, juntas e spools em escalas diferentes.</span><code>DB: três séries semanais separadas para evitar distorção visual</code></div>
        <div class="dashboard-audit-item"><strong>9 · Rundown</strong><span>Excel: peso real + planejamentos e replanejamentos manuais/obsoletos.</span><code>DB: peso real soldado × manufacture_schedule_date atual</code></div>
        <div class="dashboard-audit-item"><strong>10 · Análise de escopo</strong><span>Excel: COUNTIF no status de fabricação do BASE SPOOL.</span><code>DB: manufacture_status ativo do Spool Map</code></div>
        <div class="dashboard-audit-item"><strong>Regra de semana</strong><span>A planilha usa uma semana não ISO.</span><code>WEEKNUM(data,14)-1 mantido por compatibilidade</code></div>
      </div></details>
      <div class="dashboard-source-strip"><span><strong>Joint Traceability:</strong> ${fmt(d.moduleRows.length)} juntas</span><span><strong>Spool Map:</strong> ${fmt(d.moduleSpools.length)} spools</span><span><strong>Semana:</strong> ${escape(weekLabel(filters.week))}</span><span><strong>Soldagem na semana:</strong> ${fmt(selectedStage)}</span><span><strong>Polegadas na semana:</strong> ${fmt(d.selectedInches,1)}″</span></div>`;
    ...
    else document.querySelector('#dbValueSeries').innerHTML='<div class="db-empty"><div><strong>Sem cobertura de valores suficiente.</strong><span>Nenhum valor será inferido sem correspondência comprovada.</span></div></div>';
  }

  async function loadData(showToast) {
    const content=document.querySelector('#dashboardContent');
    if(!state.supabase.token){if(content)content.innerHTML='<div class="db-empty"><strong>Entre no painel para carregar o Dashboard.</strong></div>';return;}
    if(content)content.innerHTML='<div class="db-empty"><div><strong>Atualizando Dashboard...</strong><span>Carregando fontes e recalculando relatórios.</span></div></div>';
    try {
      ...
      if(content)content.innerHTML=`<div class="db-empty"><div><strong>Não foi possível carregar o Dashboard.</strong><span>${escape(error.message)}</span></div></div>`;
  ...
  function install() {
    if(installed)return;
    createView();
    if(!document.querySelector('#view-dashboard')){setTimeout(install,300);return;}
    installed=true;
